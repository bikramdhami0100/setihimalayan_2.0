...
export const registerForPushNotifications = async () => {
  ...
  if (Constants.isDevice) {
    ...
    if (finalStatus !== 'granted') {
      useUIStore.getState().showSnackbar('Failed to get push token for notifications!', 'error');
      return null;
    }
    token = (await Notifications.getExpoPushTokenAsync()).data;
    await setItem(STORAGE_KEYS.NOTIFICATION_TOKEN, token);
    useUIStore.getState().setNotificationPermission(true);
  } else {
    useUIStore.getState().showSnackbar('Push notifications require a physical device', 'warning');
  }
