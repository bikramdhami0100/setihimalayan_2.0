...
export default function ProfileScreen() {
  ...
  const handleChangePassword = async () => {
    ...
    if (result.success) {
      ...
      showSnackbar(result.message || 'Password change failed', 'error');
    }
  };
  ...
  // Prevent errors if user is null
  if (!user) {
    return (
    ...
    <ScrollView style={styles.container}>
      ...
      {!isEditing ? (
        <>
          ...
          <Button mode="contained" onPress={handleLogout} style={styles.logoutButton} buttonColor={colors.error}>
            Logout
          </Button>
        </>
      ) : (
...
const styles = StyleSheet.create({
  ...
  logoutButton: { marginHorizontal: 16, marginBottom: 32 },
